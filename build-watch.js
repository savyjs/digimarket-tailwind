...
// Add event listeners.
watcher
  .on('add', path => log(`File ${path} has been added`) && reBuild())
  .on('change', path => log(`File ${path} has been changed`) && reBuild())
  .on('unlink', path => log(`File ${path} has been removed`) && reBuild());
